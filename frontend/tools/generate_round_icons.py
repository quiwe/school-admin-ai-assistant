#!/usr/bin/env python3
"""
Generate Android round app icons in different densities
"""
...
def create_round_icon(size, output_path):
    """Create a round app icon with the specified size"""
    # Create a new image with RGBA mode
    img = Image.new('RGBA', (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    # Draw a blue circle background (full size for round icon)
    circle_bbox = [0, 0, size, size]
    ...
    print(f"Created round icon: {output_path} ({size}x{size})")
...
def main():
    ...
    for density, size in densities.items():
        ...
        create_round_icon(size, icon_path)
    
    print("All round icons generated successfully!")
